    ...
    <div class="col-md-12">
        <div class="form-row">
            <div class="form-group col-md-3">
                ...
                <input type="text" class="form-control datepicker" data-date-format="yyyy-mm-dd" id="tgl_adjust" name="tgl_adjust" required autocomplete="off" value="<?= date('Y-m-d'); ?>" disabled>
            </div>
            <div class="form-group col-md-3">
                <label for="no_penyesuaiaan_adjust">No. Penyesuaian</label>
                <input type="text" class="form-control" id="no_penyesuaiaan_adjust" name="no_penyesuaiaan_adjust" required disabled>
            </div>

            <div class="form-group col-md-3">
                <label for="keterangan_adjust">Keterangan</label>
                <input type="text" class="form-control" id="keterangan_adjust" name="keterangan_adjust" required value="-">
            </div>
        </div>
        <div class="form-row">
                    ...
                    <tbody id="customFields">
                        <tr>
                            ...
                            <td style="text-align:center;">
                                <input type="text" class="form-control" id="kode_adjust_0" name="kode_adjust" required onclick="handlerModalKode('0')">
                            </td>
                            <td>
                                <input type="text" class="form-control" id="barang_adjust_0" name="barang_adjust" required>
                            </td>
                            <td style="text-align:center;">
                                <input type="text" class="form-control" id="qty_adjust_0" name="qty_adjust" required>
                            </td>
                            <td style="text-align:center;">
...
<script>
    ...
    fetch('<?= base_url('Adjust_stok/getLatestNoTf'); ?>')
        .then(response => response.json())
        .then((result) => {
            $('input[name=no_penyesuaiaan_adjust]').val(result);
        });
    ...
    fetch('<?= base_url('barang/all_gudang'); ?>')
        ...
        .then((result) => {
            ...
            $('select[name=gudang_asal_adjust]').html(data);
            $('select[name=gudang_tujuan_adjust]').html(data);
        });
    ...
    const handleSubmitNewData = () => {
        ...
        if ($("#error").val() != 0) {
            ...
            const datas = {
                tgl: $("#tgl_adjust").val(),
                no_penyesuaian: $("#no_penyesuaiaan_adjust").val(),
            ...
            $.ajax({
                url: "<?= base_url('Adjust_stok/tambah'); ?>",
                type: "POST",
                data: datas,
                dataType: "JSON",
